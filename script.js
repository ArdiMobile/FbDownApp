...
urlInput.addEventListener('focus', async () => {
    ...
    } catch (e) { urlInput.placeholder = "Paste YouTube, Facebook or Instagram link..."; }
});
...
async function processPreview(url) {
    ...
    try {
        ...
        const qConfig = {
            ...
            '1440p':{gradient:'linear-gradient(145deg,#009959,#006b3d)',emoji:'💎'},
        ...
        let qualityButtons = data.formats.map((f, index) => {
            ...
            return `<button onclick="downloadVideo('\( {f.url}',' \){f.quality}')" style="display:flex;align-items:center;justify-content:space-between;width:100%;padding:\( {isBest?'13px 14px':'11px 12px'};margin-bottom:6px;background: \){q.gradient};color:#fff;border:none;border-radius:10px;font-size:\( {isBest?'13px':'12px'};font-weight:600;cursor:pointer;transition:all 0.2s;box-shadow: \){isBest?'0 4px 15px rgba(0,0,0,0.25)':'0 2px 6px rgba(0,0,0,0.1)'};" onmouseover="this.style.transform='translateY(-2px)';this.style.boxShadow='0 6px 20px rgba(0,0,0,0.3)'" onmouseout="this.style.transform='translateY(0)';this.style.boxShadow='\( {isBest?'0 4px 15px rgba(0,0,0,0.25)':'0 2px 6px rgba(0,0,0,0.1)'}'"><span style="display:flex;align-items:center;gap:6px;"><span style="font-size:15px;"> \){q.emoji}</span>
            <span>${f.quality} ${isBest?'<span style="opacity:0.8;font-weight:400;font-size:11px;">· Best</span>':''}</span></span>
            <span style="display:flex;align-items:center;gap:4px;"><i class="fas fa-download" style="font-size:13px;"></i></span></button>`;
        }).join('');
    ...
    return `
        <div class="sidebar-card">
            ...
            <div style="display:flex;flex-direction:column;gap:6px;">
                <div style="display:flex;align-items:center;gap:8px;padding:10px;background:#ffebee;border-radius:10px;border-left:3px solid #FF0000;"><i class="fab fa-youtube" style="color:#FF0000;font-size:18px;"></i><div><span style="font-size:12px;font-weight:600;color:#002611;">YouTube</span><span style="font-size:9px;color:#4a6b56;display:block;">Videos, Shorts</span></div></div>
            </div>
        </div>`;
}
