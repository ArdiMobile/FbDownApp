...
async function processPreview(url) {
    ...
    try {
        ...
        let qualityButtons = data.formats.map((f, index) => {
            ...
            return `<button onclick="downloadVideo('${f.url}','${f.quality}')" style="display:flex;align-items:center;justify-content:space-between;width:100%;padding:${isBest?'13px 14px':'11px 12px'};margin-bottom:6px;background:${q.gradient};color:#fff;border:none;border-radius:10px;font-size:${isBest?'13px':'11px'};font-weight:600;cursor:pointer;transition:all 0.2s;box-shadow:${isBest?'0 4px 15px rgba(0,0,0,0.25)':'0 2px 6px rgba(0,0,0,0.1)'};" onmouseover="this.style.transform='translateY(-2px)';this.style.boxShadow='0 6px 20px rgba(0,0,0,0.3)'" onmouseout="this.style.transform='translateY(0)';this.style.boxShadow='${isBest?'0 4px 15px rgba(0,0,0,0.25)':'0 2px 6px rgba(0,0,0,0.1)'}'"><span style="display:flex;align-items:center;gap:6px;"><span style="font-size:15px;">${q.emoji}</span><span>${f.quality} ${isBest?'<span style="opacity:0.8;font-weight:400;font-size:11px;">· Best</span>':''}</span></span><span style="display:flex;align-items:center;gap:6px;">${hasAudio?'<span style="font-size:9px;background:rgba(0,200,83,0.25);color:#00c853;padding:2px 7px;border-radius:4px;font-weight:700;">🎵 SOUND</span>':'<span style="font-size:9px;background:rgba(255,255,255,0.08);color:#aaa;padding:2px 7px;border-radius:4px;">🔇 MUTED</span>'}${f.filesize_approx?`<span style="font-size:9px;opacity:0.9;background:rgba(255,255,255,0.2);padding:2px 7px;border-radius:8px;">${(f.filesize_approx/1024/1024).toFixed(1)}MB</span>`:''}<i class="fas fa-download" style="font-size:12px;"></i></span></button>`;
        }).join('');
